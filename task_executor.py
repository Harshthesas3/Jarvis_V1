...
def _handle_open_app(plan: dict, ctx: ExecutorContext) -> str:
    ...
    try:
        ...
        if fallback_name:
            ...
            # Security: only allow safe file names for os.startfile
            safe_name = re.sub(r'[^a-zA-Z0-9_\-.\s\\:]', '', fallback_name)
...
def _clean_for_speech(text: str) -> str:
    return _CLEAN_SPEECH_RE.sub(" ", text).strip()


# Pre-compiled regex for speech cleaning
_CLEAN_SPEECH_RE = re.compile(r"[*#`_\-~]")
...
def _handle_music(plan: dict, ctx: ExecutorContext) -> str:
    ...
    # If a specific song/album is requested, open the music app and search.
    if query:
        app = (plan.get("app") or "apple music").strip().lower()
        # Open the music app first, then use search_in_app_v2 to find the song.
        return _handle_search_in_app_v2({
...
def _handle_close_app(plan: dict, ctx: ExecutorContext) -> str:
    ...
    # Security: only allow safe process names (no path separators or shell chars)
    safe_app = re.sub(r'[^a-zA-Z0-9_.-]', '', app)
    if not safe_app:
        return "Invalid application name, sir."
    try:
        subprocess.run(["taskkill", "/IM", safe_app + ".exe", "/F"],
...
def _handle_switch_window(plan: dict, ctx: ExecutorContext) -> str:
    ...
    # Security: use encoded PowerShell command with safe argument passing
    try:
        import subprocess
        # Pass target as a separate argument, not interpolated into the script
        ps_script = (
            'param($windowName); '
...
def _handle_run_program(plan: dict, ctx: ExecutorContext) -> str:
    ...
    # Security: only allow programs with safe characters, no shell metacharacters
    if not re.match(r"^[\w\-.\\/: ]+$", program):
        logger.warning("Blocked unsafe program name: %s", program)
        return f"Program name contains invalid characters, sir."
    try:
        subprocess.Popen(program, shell=False,
                         stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        return f"Running {program}, sir."
    except FileNotFoundError:
        return f"Program '{program}' was not found, sir."
    except Exception as exc:
        return f"Failed to run {program}, sir. {exc}"


_MAX_TERMINAL_COMMAND_LENGTH = 512
_BLOCKED_CMD_PATTERNS = [
    r"format\s", r"cipher\s", r"attrib\s",
    r"rmdir\s", r"rd\s/s", r"del\s/f",
    r"net\s+user", r"net\s+localgroup", r"net\s+accounts",
    r"takeown\s", r"icacls\s",
    r"reg\s+delete", r"reg\s+add", r"reg\s+import",
    r"remove-item", r"clear-content", r"move-item",
    r"rm\s+-[rR]", r"rm\s+/-[rR]",
    r"invoke-", r"start-process", r"new-object",
    r"add-type", r"add-content",
    r"set-content", r"out-file", r"write-host",
    r"stop-process", r"restart-computer", r"stop-computer",
    r"get-wmi", r"get-cim", r"remove-wmi",
    r"set-executionpolicy", r"set-itemproperty",
    r"remove-itemproperty", r"new-itemproperty",
    r"enable-", r"disable-", r"unregister-",
    r"\[system\.",
]
_BLOCKED_CMD_RE = re.compile(
    "|".join(f"(?:{p})" for p in _BLOCKED_CMD_PATTERNS),
    re.IGNORECASE,
)
...
def _handle_run_terminal_command(plan: dict, ctx: ExecutorContext) -> str:
    ...
    if len(command) > _MAX_TERMINAL_COMMAND_LENGTH:
        return "Command is too long, sir."
    # Security: only allow safe characters for PowerShell -Command
    if not re.match(r"^[\w\s\-.:\\/@]+$", command):
        logger.warning("Blocked unsafe terminal command: %s", command[:100])
        return "Command contains invalid characters, sir."
    if _BLOCKED_CMD_RE.search(command):
        logger.warning("Blocked dangerous command: %s", command[:100])
        return "That command is not allowed for security reasons, sir."
    try:
        ...
        if result.returncode != 0:
            logger.warning("Command returned error: %s", stderr)
            return f"Command returned an error, sir."
        if output:
...
# _EXEC_CTX is now an alias for the shared context_store singleton.
# All reads/writes go to one place, eliminating state divergence.
_EXEC_CTX = _cs._STORE
...
def get_execution_metrics() -> dict:
    ...
    calls = m["total_calls"]
    if calls > 0:
        m["average_duration_ms"] = m["_total_duration_ms"] // calls
    del m["_total_duration_ms"]
    return m
...
def _update_exec_context(plan: dict, tts_result: str) -> None:
    ...
    if action == "folder_operation":
        if op == "create_folder":
            name = plan.get("name", "")
            _cs.set("last_folder", name)
            _cs.set("current_folder", name)
        elif op == "rename_folder":
            new_name = plan.get("new_name", "")
            _cs.set("last_folder", new_name)
            _cs.set("current_folder", new_name)
    if action == "file_operation":
        pname = plan.get("name", "")
        ppath = plan.get("path", "")
        if op == "create_file":
            _cs.set("last_file", pname)
            _cs.set("current_file", pname)
            if folder:
                _cs.set("current_folder", os.path.basename(folder.rstrip("/\\")))
        elif op in ("open_file", "write_file", "append_file", "read_file"):
            _cs.set("last_file", ppath)
            _cs.set("current_file", ppath)
    if action == "clipboard":
        if op == "write":
            _cs.set("last_clipboard", plan.get("text", ""))
            _cs.set("clipboard_contents", plan.get("text", ""))
        elif op == "read":
            _cs.set("last_clipboard", tts_result)
    if action == "screenshot":
        _cs.set("last_screenshot", tts_result)
    if action == "web_search":
        _cs.set("last_search_result", tts_result)
        _cs.set("last_search_query", plan.get("query", ""))
    if action == "open_app":
        _cs.set("last_app", plan.get("app", ""))
        _cs.set("current_app", plan.get("app", ""))
    if action == "browser_open":
        _cs.set("last_url", plan.get("url", ""))
    if action in ("focus_window", "wait_for_window"):
        title = plan.get("title", "") or plan.get("target", "")
        if title:
            _cs.set("current_window", title)
    if action in ("search_in_app", "search_in_app_v2"):
        _cs.set("current_app", plan.get("app", ""))
        _cs.set("current_window", plan.get("app", ""))
...
def _polish_tts(text: str) -> str:
    if not text:
        return text
    text = _POLISH_SIR_RE.sub("sir", text)
    text = _POLISH_PUNCT_RE.sub(r"\1", text)
    text = _POLISH_COMMA_DOT_RE.sub(".", text)
    text = _POLISH_DOT_WORD_RE.sub(r". \1", text)
    return text.strip()


_POLISH_SIR_RE = re.compile(r"\bsir\b[,\s]*\bsir\b", re.IGNORECASE)
_POLISH_PUNCT_RE = re.compile(r"([.!?]){2,}")
_POLISH_COMMA_DOT_RE = re.compile(r",\s*\.")
_POLISH_DOT_WORD_RE = re.compile(r"\.(\w)")
...
def execute_plan(plan: dict) -> str:
    ...
    try:
        ...
        _EXEC_METRICS["last_duration_ms"] = duration_ms
        _EXEC_METRICS["_total_duration_ms"] += duration_ms