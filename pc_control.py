...
def _spawn_detached(target: str) -> tuple[bool, str]:
    ...
    try:
        if target.startswith(("http://", "https://")):
            ...
            if not re.match(r'^https?://[a-zA-Z0-9][-a-zA-Z0-9.]*\.[a-zA-Z]{2,}', target):
                logger.warning("Blocked invalid URL in _spawn_detached: %s", target)
                return False, "invalid_url"
            webbrowser.open(target)
        elif target.startswith("shell:"):
            # shell: URIs are safe (Windows-only, fixed prefixes)
