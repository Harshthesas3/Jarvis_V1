...
def _get_codegen_model() -> str:
    """Return the code generation model name from settings, with a safe default."""
    try:
        from settings_manager import settings as _s
        return _s.get("models.code_model", "qwen3.5:4b")
    except Exception:
        return "qwen3.5:4b"
...
def write_code_to_file(code: str, path: str) -> dict:
    try:
        ...
        # Security: restrict file writes to allowed directories
        allowed_dirs = [
            os.path.realpath(os.path.expanduser("~")),  # home
            os.path.realpath(os.getcwd()),               # cwd
            os.path.realpath(os.environ.get("TEMP", os.path.expanduser("~"))),
        ]
        if not any(resolved.startswith(d + os.sep) or resolved == d for d in allowed_dirs):
            logger.warning("Blocked path traversal: %s -> %s", path, resolved)
            return {"ok": False, "tts": "That path is outside allowed directories, sir."}
