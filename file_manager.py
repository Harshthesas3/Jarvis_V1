...
# Block roots - system directories that must never be touched.
_BLOCKED_PATHS: list[Path] = [
    Path(os.path.join(os.sep, "Windows")).resolve(),
    Path(os.path.join(os.sep, "Program Files")).resolve(),
    Path(os.path.join(os.sep, "Program Files (x86)")).resolve(),
    Path(os.path.join(os.sep, "ProgramData")).resolve(),
    Path(os.path.join(os.sep, "System32")).resolve(),
    Path(os.path.join(os.sep, "SysWOW64")).resolve(),
    Path(os.path.join(os.sep, "Boot")).resolve(),
]
...
def _is_blocked(p: Path) -> bool:
    """Check if the resolved path is a protected system directory (exact match)."""
    try:
        resolved = p.resolve()
        return any(resolved == blocked or str(resolved).lower().startswith(
            str(blocked).lower() + os.sep) for blocked in _BLOCKED_PATHS)
    except Exception:
        return False
